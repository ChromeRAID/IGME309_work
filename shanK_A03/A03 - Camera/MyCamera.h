...
class MyCamera
{
	...
	vector3 m_v3Rotations = ZERO_V3; //store rotation vector
	vector3 m_v3Forward = m_v3Target; //forward vector
	vector3 m_v3Right = ZERO_V3; //right vector
	vector3 m_v3Up = ZERO_V3; //up vector

	quaternion m_qOrientation = quaternion(); //quaternion to prevent gimbal lock
...
public:
	...
	*/
	...
	void Yaw(float a_fAngle, float a_fDamper = 1.0f);
	void Pitch(float a_fAngle, float a_fDamper = 1.0f);
	vector3 SetForwardVector(void);
	vector3 SetRightVector(void);
	vector3 SetUpVector(void);
};
